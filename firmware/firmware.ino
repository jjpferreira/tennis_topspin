...
static void publishBleTelemetry(uint32_t nowMs, const SensorPipelineResult& sensorResult) {
    ...
    if ((nowMs - lastNotifyMs) >= BLE_FAST_NOTIFY_INTERVAL_MS && isStreamActive(nowMs)) {
        ...
        const uint16_t instantRpmX10 = sensor.getInstantRpmX10(
            nowMs,
            g_rpmPulsesPerRev,
            static_cast<uint32_t>(KY003_RPM_FRESHNESS_MS)
        );
...
static void publishFirmwareHeartbeat(uint32_t nowMs) {
    // Repeat the firmware build identifier every 10 seconds on serial so the
    // operator can verify the running build at any point — even minutes after
    // boot — without rebooting the device.
    static uint32_t lastFwLogMs = 0;
    if ((nowMs - lastFwLogMs) < 10000u && lastFwLogMs != 0) return;
    lastFwLogMs = nowMs == 0 ? 1 : nowMs;
    Serial.print(F("[FW] "));
    Serial.println(FIRMWARE_INFO_STRING);
}
...
void setup() {
    ...
    // Banner with semver + compile-time stamp. Because FIRMWARE_BUILD_STAMP is
    // generated by the C preprocessor every build, this line MUST change on
    // every fresh flash — confirming at-a-glance that a new firmware is live.
    Serial.println();
    Serial.print(F("[BOOT] "));
    Serial.println(FIRMWARE_INFO_STRING);
